...
- Current owner-download asset: `Apollo-Windows-Beta-Build-93-Explorer-Compatible.zip`
...
byte-identical, but its ZIP paths are compatible with Windows Explorer. The
unique owner-download filename prevents confusion with the original historical
archive, which used the same generic filename as the first corrected upload.
